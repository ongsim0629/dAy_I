...
router.post("/", (req, res) => {
  ...
  db.query("INSERT INTO diary (DIARY_WRITER_ID, DIARY_WRITE_DATE, DIARY_TITLE, DIARY_CONTENT) VALUES (?, ?, ?, ?);",[check.user_id,date,title,content], function (error, results, fields) {
  ...
  ModelResult((err, {result}={})=>{
    ...
    db.query("SELECT PLAYLIST_ID, PLAYLIST_TITLE FROM playlist WHERE PLAYLIST_EMOTION = ? ORDER BY RAND() LIMIT 1;",emotion, function (error, results, fields) {
      ...
      db.query("UPDATE diary SET DIARY_EMOTION = ?,DIARY_PLAYLIST = ?, DIARY_SUMMARY = ? WHERE DIARY_WRITER_ID = ? and DIARY_WRITE_DATE = ?;",[emotion,results[0].PLAYLIST_ID,summary,check.user_id,date], function (error, results, fields) {
        if (error) throw error;
      });
    });
